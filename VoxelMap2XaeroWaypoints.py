...
def extract_xz_coords(waypoint):
    if waypoint.dim == 'the_nether':  # Check if the dimension is Nether
        return (waypoint.x // 8, waypoint.z // 8)  # Convert Nether to Overworld coords by dividing both x and z by 8
    return (waypoint.x, waypoint.z)  # Return original coordinates for Overworld and End
...
def format_waypoint_for_xaero(waypoint):
    ...
    x, z = extract_xz_coords(waypoint)  # Adjust this line to use the corrected coordinates
    return f"waypoint:{waypoint.name}:{initial}:{x}:{waypoint.y}:{z}:{color}:{boolean_to_string(not waypoint.enabled)}:0:gui.xaero_default:false:0:0"
